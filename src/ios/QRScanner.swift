...
class QRScanner : CDVPlugin, AVCaptureMetadataOutputObjectsDelegate {
    ...
    @objc func prepScanner(command: CDVInvokedUrlCommand) -> Bool{
        ...
        do {
            if (captureSession?.isRunning != true){
                ...
                metaOutput!.metadataObjectTypes = [AVMetadataObject.ObjectType.qr, AVMetadataObject.ObjectType.code128]
    ...
    func metadataOutput(_ captureOutput: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
       if metadataObjects.count == 0 || metadataObjects.count > 1 || scanning == false {
        ...
        if (found.type == AVMetadataObject.ObjectType.qr || found.type == AVMetadataObject.ObjectType.code128) && found.stringValue != nil {
            scanning = false
